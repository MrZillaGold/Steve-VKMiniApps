...
import {Panel, PanelHeader, PanelHeaderContent, Input, FormLayout, Button, PanelHeaderButton, Avatar, Group, Cell, Header, List, FormLayoutGroup, Separator} from "@vkontakte/vkui";
...
class ServerInfo extends React.Component {
    ...
    onClick() {
        ...
        axios.get(`https://stevecors.herokuapp.com/https://api.mcsrvstat.us/2/${this.state.ip}`)
            ...
            .then(data => {
                if (data.online) return this.setState({response: data, spinner: false, titleIP: this.state.ip.toLowerCase()});
    ...
    componentDidMount() {
        ...
        VKConnect.send("VKWebAppStorageGet", {"keys": ["steveFavoriteList"]})
            .then(res => {
    ...
    render() {
        ...
        const {spinner, error, response, titleIP, editFavorite, openFavorite, ip, favoriteList, scheme} = this.state;
            ...
            <Panel id={id}>
                ...
                <Online onChange={() => this.componentDidMount()}>
                    <FormLayout>
                        ...
                        <Button onClick={() => this.onClick()} size='xl' disabled={!(ip.length > 2 && !editFavorite && !spinner && (ip.match(ipRegExp1) || ip.match(ipRegExp2) || ip.match(ipRegExp3) || ip.match(ipRegExp4)))}>
