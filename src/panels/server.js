...
class ServerInfo extends React.Component {
    ...
    getInfo() {
        ...
        axios.get(`https://stevecors.herokuapp.com/https://api.mcsrvstat.us/2/${this.state.ip}`)
            ...
            .then(data => {
                if (data.online) return this.setState({data, spinner: false, titleIP: this.state.ip.toLowerCase()});

                return this.setState({error: `Сервер ${this.state.ip} оффлайн, либо информация отсутствует.`, spinner: false});
            })
            .catch(err => {
    ...
    render() {
        ...
        const { spinner, error, data, titleIP, editFavorite, openFavorite, ip, favoriteList, scheme } = this.state;
                                ...
                                <List>
                                    {
                                        favoriteList.map((item, index) => (
                                            <Cell key={item} draggable={editFavorite} removable={editFavorite}
                            ...
                            <List>
                                ...
                                </Cell>
                                {
                                    ...
                                    <Cell multiline style={{whiteSpace: "pre-wrap"}}>
                                        {
                                            data.players.list.toString().replace(/,/g, ",  ")
                                        }
                                    </Cell>
                                }
                            </List>
