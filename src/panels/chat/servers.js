...
class Servers extends React.Component {
    ...
    async componentDidMount() {
        ...
        if (serversStorage.length > 1) {
            await this.setState({servers: JSON.parse(serversStorage)});
        }
                    ...
                    <Group style={{marginBottom: "70px"}}>
                        {
                            servers.length > 0 ?
                                ...
                                <Cell multiline before={<Icon24Add height={44} width={44}/>} size="m"
                                      ...
                                      onClick={() => navigator.showModal("add-server", {addServer, servers: servers })}>
                    ...
                    <FixedLayout vertical="bottom" style={{display: "flex", direction: "rtl"}}>
                        {
                            editList ?
                                <div style={{display: "flex", marginBottom: "10px"}}>
                                    ...
                                    <div className="footer-icon">
                                        <Icon24Cancel onClick={() => this.setState({editList: false, servers: serversBackup})} className="footer-icon__icon" height={35} width={35}/>
                                    </div>
                                </div>
                                :
                                <div style={{display: "flex", marginBottom: "10px"}}>
                                    <div className="footer-icon">
                                        <Icon24Add className="footer-icon__icon" onClick={() => servers.length < 21 ? navigator.showModal("add-server", {addServer, servers: servers }) : this.props.error("Нельзя добавить больше 20 серверов!")} height={35} width={35}/>
                                    </div>
                                    {
                                        servers.length > 0 &&
                                        <div className="footer-icon">
                                            <Icon28EditOutline onClick={() => this.setState({editList: true, serversBackup: servers})} className="footer-icon__icon" height={35} width={35}/>
                                        </div>
                                    }
                                </div>
                        }
