...
class ServerInfoGet extends React.Component {
    ...
    onClick() {
        ...
        axios.get(`https://stevecors.herokuapp.com/https://api.mcsrvstat.us/1/${this.state.ip}`).then(res => {
            ...
            if (data.offline) {
                this.setState({error: `Сервер ${this.state.ip} оффлайн!`, spinner: null});
            } else {
            ...
            <Panel id={id}>
                ...
                <Online>
                    <FormLayout>
                        ...
                        />
                        {
                            this.state.ip.length > 2 && this.state.spinner === null && this.state.ip.match(/[а-яА-ЯёЁa-zA-Z0-9]+(\.[а-яА-ЯёЁa-zA-Z0-9]+)+.*/g) ?
